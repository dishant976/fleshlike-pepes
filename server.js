...
app.get("/tokens", async (req, res) => {
  try {
    ...
    if (start >= totalSupply) return res.status(400).json({ error: "Page out of range" });
...
app.get("/token/:id/image", async (req, res) => {
  try {
    ...
    if (!svg) {
      ...
      if (!meta.image) return res.status(404).json({ error: "No image in token metadata" });
...
app.get("/owner/:address", async (req, res) => {
  try {
    ...
    if (balance === 0) return res.json({ address, balance: 0, tokens: [] });
    const idPromises = Array.from({ length: balance }, (_, i) => contract.tokenOfOwnerByIndex(address, i));
    const tokenIds = await Promise.all(idPromises);
    const tokens = await Promise.all(tokenIds.map((id) => getTokenMeta(Number(id))));
...
app.get("/traits", (req, res) => {
  const traits = ["aloha","background","bellybutton","bridged","button","count","development","dollars","electricaltape","eyeball","face","gaze","iris","looking","markings","mouth","mucousmembrane","nip","nips","pupil","shirt","shorts","skin","snot","spittle","spot","stripe","tail","tooth","wrag"];
  res.json({ traits });
});
